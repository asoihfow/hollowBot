...
module.exports = {
    ...
    run: (client, message, args) =>{
        ...
        if(!member){
            message.channel.send("vc tem que mencionar alguem")
        } else{
            if(member.id === message.author.id) {
                let killEmbed1 = new Discord.MessageEmbed()
          .setTitle(`${message.author.username} não morda vc mesmo(a)`)
         ...
         let killEmbed = new Discord.MessageEmbed()
          .setTitle(`${message.author.username} mordeu ${member.user.username}`)
          .setImage(String([images[image]]))
          .setColor(0xff0000)
